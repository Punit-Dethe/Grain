...
| History | Audio saving is detached, and database writes are queued (`ContentView.swift:3351`, `TranscriptionHistoryStore.swift:553`). | Paste completes before WAV export and SQLite history/cleanup; Flow still waits for persistence before signalling pipeline completion (`grain_actions.rs:715`, `:750`). | Text insertion no longer competes with the journal-to-WAV copy. Fully detaching persistence would allow overlapping journal saves and needs bounded ordering and shutdown draining. |
...
- The second pass restores system output mute after the recorder drains and
  waits for the main-thread paste callback before starting WAV/history work.
  Neither step adds a deliberate wait before text insertion.
