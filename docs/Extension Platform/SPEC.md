...
| **✅ 4** | Extension-owned serialized sessions with a bounded/cancellable slow stage and host-rendered owner indicator; exact-host `net:<host>` proxy with redirect/size/time limits; write-only secret settings in Grain's existing credential file; 128 MiB reported-JS-heap ceiling with three-strike disable; developer-only Tier-C companion supervisor on the existing authenticated protocol; checked voice-note acceptance extension. **Native extensions remain non-distributable until 5A.** |
