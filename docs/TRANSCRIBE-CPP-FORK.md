...
- [Fork CI for the exact pin](https://github.com/Punit-Dethe/transcribe.cpp/actions/runs/36242159304) runs on hosted Windows/Linux/macOS: native suite/CLI, safe Rust source build, generated ABI/format checks, package audit, shared dynamic consumption, Apple Metal compilation and a Linux shared source archive without `.git`. Check the run for the exact pinned SHA before promotion; earlier failed generator checks were fixed for UTF-8, CRLF and Clang enum representation differences.
