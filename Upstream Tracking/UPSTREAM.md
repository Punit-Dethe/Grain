...
## Repository layout (since the phase-7 folder move)

```
src-tauri/src/handy/   Handy's tree, verbatim + small marked [GRAIN] hooks.
                       Declared from lib.rs with #[path = "handy/..."], so crate
                       paths (crate::actions, crate::managers::…) and the file
                       CONTENTS are unchanged — upstream diffs and merges still
                       work, mapping in by rename detection (verified R100).
                       DON'T ADD FEATURES HERE.
src-tauri/src/         Grain's composition root (lib.rs, main.rs) + Grain-owned
                       modules: grain_* files, agent, bridge, rolling, the
                       routers, grain_space, master_key, prompt_record,
                       voice_actions.
crates/                Grain crates (grain-core, grain-pill, provider-router,
                       rolling-window).
```

When upstream touches a file, the merge lands it inside `handy/` automatically.
If rename detection ever fails on a big upstream refactor, fall back to
`git merge -s subtree` or map the file by hand.
...
Then regenerate the divergence budgets against the new base and commit them
with the close-out: `python "Handy Isolation/ratchet.py" --update` (the
Divergence Ratchet CI check measures against the merge base, so a close-out
shifts every number).
