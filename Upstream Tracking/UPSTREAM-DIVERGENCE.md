...
| `audio_toolkit/audio/resampler.rs` | Grain **bug fix** upstream lacks: `finish()` drains the `FftFixedIn` delay line, so the last ~`output_delay()` frames aren't silently dropped (audible as a clipped tail word on cloud STT). Candidate to upstream | Keep Grain's fix; re-thread if upstream rewrites `finish()` |
