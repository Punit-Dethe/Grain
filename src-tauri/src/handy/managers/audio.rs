...
/// regular selections stay distinct so a missing clamshell-only device cannot
/// erase the user's ordinary microphone preference.
...
struct MicrophoneResolution {
    ...
    /// Set only after successful enumeration proves a regular selection is
    /// absent. Enumeration errors must never erase persisted settings.
    unavailable_selected_microphone: Option<String>,
}

fn confirmed_unavailable_selection(
    selected_microphone: Option<String>,
    enumeration_succeeded: bool,
    device_found: bool,
) -> Option<String> {
    if enumeration_succeeded && !device_found {
        selected_microphone
    } else {
        None
    }
...
impl AudioRecordingManager {
    ...
    fn resolve_microphone_device(&self, settings: &AppSettings) -> MicrophoneResolution {
        ...
        // Only call a selection unavailable when enumeration itself succeeds.
        // A transient backend error must not destroy the user's preference.
        ...
        MicrophoneResolution {
            unavailable_selected_microphone: confirmed_unavailable_selection(
                selected_microphone,
    ...
    /// Persist fallback only after the default stream opened successfully.
    /// Re-reading prevents recovery from clearing a concurrent user selection.
...
#[cfg(test)]
mod microphone_fallback_tests {
    use super::confirmed_unavailable_selection;

    #[test]
    fn only_successful_enumeration_can_clear_a_missing_selection() {
        let name = Some("USB microphone".to_string());
        assert_eq!(
            confirmed_unavailable_selection(name.clone(), true, false),
            name
        );
        assert_eq!(
            confirmed_unavailable_selection(Some("USB microphone".into()), false, false),
            None
        );
        assert_eq!(
            confirmed_unavailable_selection(Some("USB microphone".into()), true, true),
            None
        );
    }
}
