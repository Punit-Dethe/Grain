...
pub fn run(cli_args: CliArgs) {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            if headless_mode {
                ...
                if let Some(golden) = eval_request.clone() {
                    ...
                    std::thread::spawn(move || {
                        ...
                        crate::grain_embed::shutdown_engine();
                        use std::io::Write;
            ...
            // RAM if the app is started hidden. Upstream's eager builder lives
            // there instead; its #1778 `.maximizable(true)` is already applied.
...
// Keep non-visual binding generation on the same registry the application uses.
fn command_bindings() -> Builder<tauri::Wry> {
    Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            grain_embed::grain_embed_model_status,
            grain_embed::grain_embed_download_model,
            grain_embed::grain_embed_cancel_download,
            grain_embed::grain_embed_uninstall_model,
        ...
        .events(collect_events![
            grain_embed::GrainEmbedModelProgress,
            grain_embed::GrainEmbedModelComplete,
            grain_embed::GrainEmbedModelCancelled,
            grain_embed::GrainEmbedModelError,
...
mod binding_export {
    #[test]
    #[ignore = "writes the generated frontend bindings; run explicitly after command changes"]
    fn export_bindings() {
        super::command_bindings()
            .export(
                super::Typescript::default().bigint(super::BigIntExportBehavior::Number),
                concat!(env!("CARGO_MANIFEST_DIR"), "/../src/app/bindings.ts"),
            )
            .expect("Failed to export typescript bindings");
    }
}
