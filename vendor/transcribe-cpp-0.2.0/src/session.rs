...
/// Hard bounds for one native TDT Flow run.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TdtFlowOptions {
    /// Maximum samples in any recomputed `[left | owned | right]` window.
    pub max_window_samples: usize,
    /// Permit a caller-reviewed Parakeet TDT artifact whose GGUF predates the
    /// `stt.capability.tdt_flow` key. The native layer still rejects every
    /// non-Parakeet or non-TDT head. Default-off preserves fail-closed probing.
    pub allow_unadvertised_tdt_head: bool,
}

impl Default for TdtFlowOptions {
    fn default() -> Self {
        Self {
            max_window_samples: 30 * 16_000,
            allow_unadvertised_tdt_head: false,
        }
    }
}

/// One absolute-timeline TDT Flow descriptor.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TdtFlowWindow {
    pub sequence: u64,
    pub context_start_sample: u64,
    pub fresh_start_sample: u64,
    pub commit_end_sample: u64,
    pub context_end_sample: u64,
    pub final_window: bool,
}

/// Owned output from one successfully committed descriptor.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TdtFlowUpdate {
    /// Newly committed model-native text only; never transcript history.
    pub text_delta: String,
    pub sequence: u64,
    pub committed_end_sample: u64,
    pub final_window: bool,
}
...
impl Session {
    ...
    /// Begin a bounded TDT Flow run. By default the loaded model must explicitly
    /// support [`Feature::TdtFlow`](crate::Feature::TdtFlow); a caller may opt an
    /// independently reviewed legacy-metadata artifact in through
    /// [`TdtFlowOptions::allow_unadvertised_tdt_head`]. Native code still
    /// requires a Parakeet TDT decoder head. Generic streaming support is
    /// neither required nor consulted.
    pub fn tdt_flow(&mut self, run: &RunOptions, options: &TdtFlowOptions) -> Result<TdtFlow<'_>> {
        let (run_params, _lang, _target, _family) = build_run_params(run)?;
        let max_window_samples = i32::try_from(options.max_window_samples).map_err(|_| {
            Error::InvalidArgument(format!(
                "TDT Flow max window {} exceeds i32::MAX",
                options.max_window_samples
            ))
        })?;
        let mut native_options: sys::transcribe_tdt_flow_params = unsafe { std::mem::zeroed() };
        unsafe { sys::transcribe_tdt_flow_params_init(&mut native_options) };
        native_options.max_window_samples = max_window_samples;
        native_options.allow_unadvertised_tdt_head = options.allow_unadvertised_tdt_head;
        ...
        let mut out = std::ptr::null_mut();
        {
            ...
            if *guard {
                return Err(Error::Busy(
                    "a stream or TDT Flow run is already active on this model".into(),
                ));
            }
            check(
                unsafe {
                    sys::transcribe_tdt_flow_begin(self.ptr, &run_params, &native_options, &mut out)
                },
                "TDT Flow begin",
            )?;
            if out.is_null() {
                return Err(Error::Other(
                    "TDT Flow begin succeeded without a native handle".into(),
                ));
            }
            *guard = true;
        }
...
/// An active bounded native TDT Flow run borrowed from a [`Session`].
///
/// The handle owns no PCM or transcript history. Every successful operation
/// returns an owned text delta copied before the next native mutation.
pub struct TdtFlow<'a> {
    session: &'a mut Session,
    ptr: *mut sys::transcribe_tdt_flow,
    holds_lease: bool,
    finished: bool,
}

impl std::fmt::Debug for TdtFlow<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TdtFlow")
            .field("finished", &self.finished)
            .field("holds_lease", &self.holds_lease)
            .finish()
    }
}

// SAFETY: the handle exclusively borrows a Send Session and all native calls
// require &mut self. It is intentionally not Sync.
unsafe impl Send for TdtFlow<'_> {}

impl Drop for TdtFlow<'_> {
    fn drop(&mut self) {
        let mut guard = self
            .session
            .model
        ...
        if !self.ptr.is_null() {
            unsafe {
                sys::transcribe_tdt_flow_reset(self.ptr);
                sys::transcribe_tdt_flow_free(self.ptr);
            }
...
impl TdtFlow<'_> {
    /// Exact PCM samples per encoder frame for this loaded flow.
    pub fn encoder_stride_samples(&self) -> Result<usize> {
        if self.ptr.is_null() || self.finished || !self.holds_lease {
            return Err(Error::InvalidArgument(
                "TDT Flow is no longer active".into(),
            ));
        }
        let mut stride = 0i32;
        check(
            unsafe { sys::transcribe_tdt_flow_encoder_stride_samples(self.ptr, &mut stride) },
            "TDT Flow encoder stride",
        )?;
        usize::try_from(stride)
            .ok()
            .filter(|value| *value > 0)
            .ok_or_else(|| Error::Other("TDT Flow returned an invalid encoder stride".into()))
    }

    /// Recompute and commit one strict ownership descriptor.
    pub fn process(&mut self, pcm: &[f32], window: &TdtFlowWindow) -> Result<TdtFlowUpdate> {
        if self.ptr.is_null() || self.finished || !self.holds_lease {
            return Err(Error::InvalidArgument(
                "TDT Flow is no longer active".into(),
            ));
        ...
        let n = clamp_len(pcm.len())?;
        let to_i64 = |name: &str, value: u64| {
            i64::try_from(value)
                .map_err(|_| Error::InvalidArgument(format!("TDT Flow {name} exceeds i64::MAX")))
        };
        let mut native_window: sys::transcribe_tdt_flow_window = unsafe { std::mem::zeroed() };
        unsafe { sys::transcribe_tdt_flow_window_init(&mut native_window) };
        native_window.sequence = window.sequence;
        native_window.context_start_sample =
            to_i64("context_start_sample", window.context_start_sample)?;
        native_window.fresh_start_sample = to_i64("fresh_start_sample", window.fresh_start_sample)?;
        native_window.commit_end_sample = to_i64("commit_end_sample", window.commit_end_sample)?;
        native_window.context_end_sample = to_i64("context_end_sample", window.context_end_sample)?;
        native_window.final_window = window.final_window;

        let mut native_update: sys::transcribe_tdt_flow_update = unsafe { std::mem::zeroed() };
        unsafe { sys::transcribe_tdt_flow_update_init(&mut native_update) };
        let status = {
            let _guard = self
                .session
                .model
                ...
                .unwrap_or_else(|e| e.into_inner());
            unsafe {
                sys::transcribe_tdt_flow_process(
                    self.ptr,
                    pcm.as_ptr(),
                    n,
                    &native_window,
        ...
        check(status, "TDT Flow process")?;

        let text_delta = if native_update.text_delta_bytes == 0 {
            String::new()
        } else {
            if native_update.text_delta.is_null() {
                return Err(Error::Other(
                    "TDT Flow returned a null non-empty text delta".into(),
                ));
            }
            let bytes = unsafe {
                std::slice::from_raw_parts(
                    native_update.text_delta.cast::<u8>(),
                    native_update.text_delta_bytes,
                )
            };
            std::str::from_utf8(bytes)
                .map_err(|_| Error::Other("TDT Flow returned invalid UTF-8".into()))?
                .to_owned()
        };
        Ok(TdtFlowUpdate {
            text_delta,
            sequence: native_update.sequence,
            committed_end_sample: u64::try_from(native_update.committed_end_sample).map_err(
                |_| Error::Other("TDT Flow returned a negative committed cursor".into()),
            )?,
            final_window: native_update.final_window,
        })
    }

    /// Finish after a successfully committed final descriptor. Idempotent.
    pub fn finish(&mut self) -> Result<()> {
        if self.finished {
            return Ok(());
        }
        if self.ptr.is_null() || !self.holds_lease {
            return Err(Error::InvalidArgument(
                "TDT Flow is no longer active".into(),
            ));
        }
        let mut guard = self
            ...
            .unwrap_or_else(|e| e.into_inner());
        check(
            unsafe { sys::transcribe_tdt_flow_finish(self.ptr) },
            "TDT Flow finish",
        )?;
        *guard = false;
        self.holds_lease = false;
    ...
    /// Abandon this run and immediately release native state and model lease.
    /// Idempotent; dropping afterward is a no-op.
    ...
    /// Whether the most recent native operation observed cancellation.
    pub fn was_aborted(&self) -> bool {
