            ...
            case HostHeadKind::TDT: {
                if (params != nullptr && params->family != nullptr &&
                    ...
                    st = decode_tdt_fluid_window(pm->host_decoder,
                                                 enc + static_cast<size_t>(start) * static_cast<size_t>(d_enc), count,
                                                 d_enc, pc->n_threads, ext->finalize_tail, pc->raw_tokens);
                    if (st == TRANSCRIBE_OK) {
...
transcribe_status run_validate(const transcribe_session * session, const transcribe_run_params * params) {
    ...
    if (const transcribe_status st =
            transcribe_ext_check(params->family, TRANSCRIBE_EXT_KIND_PARAKEET_TDT_WINDOW,
