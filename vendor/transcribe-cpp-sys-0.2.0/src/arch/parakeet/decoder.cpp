...
void TdtDecoderState::reset(int n_layers, int pred_hidden) {
    lstm_state.reset(n_layers, pred_hidden);
    prev_token_id = -1;
    duration_skip = 0;
}
...
void advance_tdt_cursor(int duration, int max_symbols,
                        int & step, int & new_symbols) {
    step += duration;
    new_symbols += 1;
    if (duration != 0) {
        new_symbols = 0;
...
transcribe_status decode_tdt_greedy_stateful(const HostDecoderWeights & w,
                                             const float *              enc_out,
                                             int                        T_enc,
                                             int                        d_enc,
                                             TdtDecoderState &          state_io,
                                             int64_t                    frame_offset,
                                             int                        n_threads,
                                             std::vector<TdtToken> &    out_tokens,
                                             DecoderAbortCallback       abort_callback,
                                             void *                     abort_userdata) {
    if (enc_out == nullptr || T_enc <= 0 || d_enc <= 0 || frame_offset < 0) {
        return TRANSCRIBE_ERR_INVALID_ARG;
    }
    if (w.head_kind != HostHeadKind::TDT) {
        log_msg(TRANSCRIBE_LOG_LEVEL_ERROR,
                "parakeet decoder (tdt-stateful): decode requires a TDT head");
    ...
    if (n_dur <= 0 || frame_offset > static_cast<int64_t>(INT_MAX) - T_enc) {
        return TRANSCRIBE_ERR_INVALID_ARG;
    }
    ...
    // Work on a complete copy. Neither predictor state nor output tokens are
    // visible to the caller until the whole slice succeeds.
    TdtDecoderState working = state_io;
    bool shape_ok = static_cast<int>(working.lstm_state.h.size()) == n_layers &&
                    static_cast<int>(working.lstm_state.c.size()) == n_layers;
    if (shape_ok) {
        for (int l = 0; l < n_layers; ++l) {
            if (static_cast<int>(working.lstm_state.h[static_cast<size_t>(l)].size()) != H ||
                static_cast<int>(working.lstm_state.c[static_cast<size_t>(l)].size()) != H) {
                shape_ok = false;
                break;
            }
        }
    ...
    if (!shape_ok) {
        working.reset(n_layers, H);
    }
    if (working.prev_token_id < -1 || working.prev_token_id >= n_token_cls || working.duration_skip < 0) {
        return TRANSCRIBE_ERR_INVALID_ARG;
    }

    LstmState state = working.lstm_state;
    LstmState next_state;
    next_state.reset(n_layers, H);
    std::vector<TdtToken> delta_tokens;
    ...
    int last_token  = working.prev_token_id;
    int step        = working.duration_skip;
    int new_symbols = 0;
    ...
    while (step < T_enc && iter < max_iters) {
        if (abort_callback != nullptr && abort_callback(abort_userdata)) {
        ...
        if (!is_blank) {
            ...
            tok.id              = pred_token;
            tok.p               = p;
            tok.step_at_emit    = static_cast<int>(frame_offset + step);
            tok.duration_frames = duration;
            delta_tokens.push_back(tok);
        ...
        advance_tdt_cursor(duration, w.tdt_max_symbols, step, new_symbols);
        if (duration == 0 && is_blank && w.tdt_max_symbols > 0 && new_symbols != 0) {
    ...
    log_msg(TRANSCRIBE_LOG_LEVEL_DEBUG,
            ...
            iter, static_cast<int>(delta_tokens.size()), T_enc, t_enc_proj_us / 1000.0, t_pred_us / 1000.0,
    ...
    working.lstm_state   = std::move(state);
    working.prev_token_id = last_token;
    working.duration_skip = std::max(0, step - T_enc);
    out_tokens.insert(out_tokens.end(), delta_tokens.begin(), delta_tokens.end());
    state_io = std::move(working);
    return TRANSCRIBE_OK;
}
...
transcribe_status decode_tdt_greedy(const HostDecoderWeights & w,
    ...
    state.reset(static_cast<int>(w.predictor.lstm.size()), w.predictor.pred_hidden);
    return decode_tdt_greedy_stateful(w, enc_out, T_enc, d_enc, state, 0, n_threads, out_tokens);
}
