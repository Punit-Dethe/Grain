...
// Bounded persistent state for partitioned TDT decode. It contains only the
// predictor state, previous non-blank token, and the residual duration jump
// that crossed the previous owned-slice boundary. It never owns token,
// transcript, encoder, or audio history.
...
using DecoderAbortCallback = bool (*)(void * userdata);
...
// Production TDT duration/stuck transition, exposed internally for deterministic
// partition tests. The caller owns the optional blank fast-forward optimization.
...
// Decode one owned encoder slice while carrying only bounded predictor state.
// `state_io` and `out_tokens` are committed only after the entire slice
// succeeds. A duration jump beyond T_enc is stored in duration_skip and
// consumed before the next slice. `frame_offset` is the absolute frame index
// of enc_out[0]. The optional callback is polled between greedy iterations.
transcribe_status decode_tdt_greedy_stateful(const HostDecoderWeights & w,
                                             ...
                                             int                        n_threads,
                                             std::vector<TdtToken> &    out_tokens,
                                             DecoderAbortCallback       abort_callback = nullptr,
