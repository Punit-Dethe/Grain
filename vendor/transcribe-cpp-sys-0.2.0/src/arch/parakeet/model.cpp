...
transcribe_status load(Loader & loader, const transcribe_model_load_params * params, transcribe_model ** out_model) {
    ...
    // Grain's bounded TDT Flow path is an explicit artifact/runtime
    // capability, independent of generic streaming. Missing metadata is false;
    // a wrong type or a claim on a non-TDT head fails closed at load.
    bool tdt_flow_capability = false;
    if (const transcribe_status st = read_optional_bool_kv(loader.gguf(), "stt.capability.tdt_flow", "parakeet",
                                                           false, tdt_flow_capability);
        st != TRANSCRIBE_OK) {
        return st;
    }
    if (tdt_flow_capability) {
        if (m->hparams.head_kind != HeadKind::TDT) {
            log_msg(TRANSCRIBE_LOG_LEVEL_ERROR,
                    "parakeet: stt.capability.tdt_flow=true requires a TDT decoder head");
            return TRANSCRIBE_ERR_GGUF;
        }
        transcribe::set_feature(m.get(), TRANSCRIBE_FEATURE_TDT_FLOW, true);
    }
