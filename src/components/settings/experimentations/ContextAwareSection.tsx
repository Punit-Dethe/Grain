  ...
  const handleSave = () => {
    ...
    const value =
      form.matchKind === "process"
        ...
        : trimmedValue.replace(/^https?:\/\//i, "").replace(/\/.*$/, "").replace(/^www\./i, "").toLowerCase();
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsGroup
        ...
        description="Adapts post-processing to the app you're dictating into. It softly nudges tone and vocabulary (an IDE keeps technical terms; chat stays casual; email gets slightly more polished) without hard-reformatting. Applied on top of your selected post-processing prompt. Requires post-processing to be on."
      >
        <ToggleSwitch
      ...
      <SettingsGroup
        title="Modes"
        description="Your own instructions applied only inside a specific app or website (e.g. an 'X post' mode on x.com). This is where hard formatting lives — you define it. Modes only run while context awareness is on."
      >
        {/* Add / edit form */}
        <div className={`px-4 py-3 space-y-3 ${enabled ? "" : "opacity-60"}`}>
          {/* Target kind segmented control. */}
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, matchKind: "process" }))}
            ...
            >
              <AppWindow width={15} height={15} /> Application
            </button>
            <button
              type="button"
              onClick={() => setForm((f) => ({ ...f, matchKind: "url_host" }))}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm transition-colors cursor-pointer ${
            ...
            >
              <Globe width={15} height={15} /> Website
            </button>
          </div>
          ...
          <Input
            ...
            placeholder="Mode name, e.g. X post, Work email"
          />

          {/* Templates. */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs text-ink-soft">Templates:</span>
            {TEMPLATES.map((tpl) => (
              <button
                key={tpl.key}
                type="button"
                onClick={() => applyTemplate(tpl)}
                className="px-2.5 py-1 rounded-md text-xs border border-[color-mix(in_srgb,var(--color-mid-gray)_40%,transparent)] text-ink-soft hover:text-ink hover:border-[var(--color-accent)] transition-colors cursor-pointer"
              >
                {tpl.name}
              </button>
            ))}
