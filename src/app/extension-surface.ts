...
const BRIDGE = `<script>(function(){
  ...
  window.grain = {
    ...
    auth: {
      status: function(){ return call("auth.status", {}); },
      connect: function(){ return call("auth.connect", {}); },
      disconnect: function(){ return call("auth.disconnect", {}); }
    },
    // [GRAIN] Read-only until extension mutations have a user-owned approval
    // channel. Never add a write here that returns approval credentials to the
    // worker.
    notes: {
