...
| `grain_llm_client.rs` (2026-08-08) | Upstream's `stream: false` post-processing guard (#2211da65, landed in the inert `llm_client.rs`) threaded into `ChatCompletionRequest` at all four call sites — Grain parses full JSON on every path. Upstream's #1823 reqwest source-chain diagnostics were NOT ported (Grain's multi-provider client has independent error handling; non-correctness) | Grain-owned client; port only relevant provider fixes by hand |
