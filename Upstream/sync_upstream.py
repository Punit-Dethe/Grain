...
so a verdict recorded without re-running this script silently disagreed with it.

The one output, `data.json`, is a gitignored build product regenerated on every
run. It is verdict.py's input (SHA validation + the pending list) and the source
of the ancestry-drift report CI reads. (A GitHub Pages dashboard also rendered
it until 2026-08-10, when it was retired as redundant with preflight.py and the
CI job summary — see UPSTREAM.md.)
"""
...
def write_outputs(data):
    """Write the commit ledger (data.json).

    The ledger is verdict.py's input: it validates SHAs against the tracked
    commit list and lists what still needs a verdict. (It used to also feed a
    GitHub Pages dashboard, retired 2026-08-10 as redundant with preflight.py
    and the CI job summary — see Upstream/UPSTREAM.md.)
    """
...
def refresh():
    ...
    # Prints the ancestry audit (incl. the `ALREADY APPLIED` drift block CI
    # greps for to suppress the auto-sync PR).
    report_ancestry(ledger)
